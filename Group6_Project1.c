...
void moveArm(float arm_angle){
		...
		motortimedown = 21*arm_amount;
		while (motortimedown > 0) {
		motor[armMotor] = 20;
		wait1Msec(1);
		motortimedown = motortimedown - 1;
			}
		}
		*/
...
void positionArm(float x_arm, float y_arm, float z_arm) {
	...
	if (solution_theta2pos < solution_theta2neg) {
		...
		solution_d2 = solution_d2neg;
		solution_theta2 = solution_theta2neg;
	}

	rotate(solution_theta1);
	translate(solution_d2);
